...
class RT1(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.max_eps_dict = max_eps_dict
        self.z_dict = nn.ParameterDict()

        for task_str, max_eps in self.max_eps_dict.items():
    ...
    def head(
        ...
        # xg = torch.cat([x, enc_feat[0]], dim=1)
        # xg = einops.rearrange(
        #     xg, '(bpad n) c h w -> bpad (n c) h w', n=N
        # )
        # xg = self.quat_decoder(xg)
        # xt_offset = xg[..., :3]
        # # xt_offset = z_offset[padding_mask]
        
        # rotation = normalise_quat(xg[..., 3:7])
        # gripper = xg[..., 7].unsqueeze(-1)

        # action = torch.cat([position + xt_offset, rotation, gripper], dim=-1)
        ...
        # z_offset = z_offset[padding_mask]
        # xp_offset = self.offset(z_offset)
        xp_offset = z_offset[padding_mask]
        position += xp_offset
    ...
    def forward(
        ...
        logits = self.head(pooled, video, pc_obs, enc_feat, padding_mask, text_embed, z_offset)
        return logits
