...
from structures import CameraName, Instruction
# from instruction import get_language_feat
...
class RLBench(data.Dataset):
    ...
    def __getitem__(self, episode_id: int) -> Optional[Sample]:
        ...
        instr: torch.Tensor = episode[5][0]
        ...
        attn_indices = _attn_indices_to_tensor(episode[3], frame_ids, self._cameras)
...
class RLBench_RT1(data.Dataset):
    """
    RLBench dataset, 10 tasks
    """
    ...
    def __init__(
        ...
        root: Union[Path, str, List[Path], List[str]],
        taskvar: List[Tuple[str, int]],
        instructions: Optional[Instruction],
        max_episode_length: int,
        gripper_pose: GripperPose = None,
        taskvar_token: bool = True,
        cache_size: int = 200,
        max_episodes_per_taskvar: int = 100,
        # num_iters: Optional[int] = None,
        jitter: bool = False,
        # cameras: Tuple[CameraName, ...] = ("front",),
        cameras: Tuple[CameraName, ...] = ("wrist", "left_shoulder", "right_shoulder"),
        training: bool = True,
    ):
        self._cache = Cache(cache_size, loader)
        ...
        if instructions is None:
            ...
            # self.all_instr = []
            for task, var in taskvar:
                self._instructions[task][var] = instructions[task][str(var)]
                # self.all_instr.extend(list(self._instructions[task][var]))
    ...
    def __getitem__(self, episode_id: int) -> Optional[Sample]:
        ...
        if states.shape[-1] != 128 or states.shape[-2] != 128:
            raise ValueError(f"{states.shape} {self._episodes[episode_id]}")
        # if states.shape[-1] != 256 or states.shape[-2] != 256:
        #     raise ValueError(f"{states.shape} {self._episodes[episode_id]}")
        pad_vec = [0] * (2 * states.dim())
        pad_vec[-1] = pad_len
        states = F.pad(states, pad_vec)
        ...
        # masks = states[:, :, 2]
        # masks = einops.reduce(masks, 't n c h w -> t n 1 h w', "mean")
        # rgbs = torch.cat([rgbs, masks], 2)
        ...
        if self._task_to_id is not None:
            taskvar = torch.Tensor([self._task_to_id[task], variation])
            one_hot = np.zeros(10)
            one_hot[self._task_to_id[task]] = 1
            one_hot = torch.Tensor(one_hot)
            one_hot = einops.repeat(one_hot, 'n -> t n', t = len(frame_ids))
            shape = [0, 0] * one_hot.dim()
            shape[-1] = pad_len
            one_hot = F.pad(one_hot, tuple(shape), value=0)
        else:
            taskvar = None
